CLASS LHC_ZKJAN_I_ERRORCODE_S DEFINITION INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
      AUGMENT FOR MODIFY
        IMPORTING
          ENTITIES_CREATE FOR CREATE KjanErrorCodeAll\_KjanErrorCode
          ENTITIES_UPDATE FOR UPDATE KjanErrorCode.
ENDCLASS.

CLASS LHC_ZKJAN_I_ERRORCODE_S IMPLEMENTATION.
  METHOD AUGMENT.
    DATA: text_for_new_entity      TYPE TABLE FOR CREATE ZKjan_I_ErrorCode\_KjanErrorCodeText,
          text_for_existing_entity TYPE TABLE FOR CREATE ZKjan_I_ErrorCode\_KjanErrorCodeText,
          text_update              TYPE TABLE FOR UPDATE ZKjan_I_ErrorCodeText.
    DATA: text_tky_link  TYPE STRUCTURE FOR READ LINK ZKjan_I_ErrorCode\_KjanErrorCodeText,
    ...
    LOOP AT entity-%TARGET ASSIGNING FIELD-SYMBOL(<target>).
      INSERT VALUE #( %CID_REF = <target>-%CID
                      %IS_DRAFT = <target>-%IS_DRAFT
                        %KEY-ErorrCode = <target>-%KEY-ErorrCode
                      %TARGET = VALUE #( (
    ...
    ENDLOOP.
    MODIFY AUGMENTING ENTITIES OF ZKjan_I_ErrorCode_S
      ENTITY KjanErrorCode
        CREATE BY \_KjanErrorCodeText
        FROM text_for_new_entity.

    IF entities_update IS NOT INITIAL.
      READ ENTITIES OF ZKjan_I_ErrorCode_S
        ENTITY KjanErrorCode BY \_KjanErrorCodeText
